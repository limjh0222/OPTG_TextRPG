...
{   // 싱글톤
    ...
    public PlayerData InitJob(string name,int job)
    {
        ...
        {   //이름, 레벨, 공격력, 방어력, MaxHP ,MaxMP ,골드
            case 1:
                return new PlayerData(name, "전사", 1, 25, 20, 200, 50, 1000,SkillManager.Instance.GetSkillsForJob(1));
            case 2:
                return new PlayerData(name, "마법사", 1, 12, 10, 100, 200, 1000, SkillManager.Instance.GetSkillsForJob(2));
            case 3:
                return new PlayerData(name, "도적", 1, 15, 13, 150, 75, 1000, SkillManager.Instance.GetSkillsForJob(3));
            case 4:
                return new PlayerData(name, "궁수", 1, 17, 15, 130, 100, 1000,  SkillManager.Instance.GetSkillsForJob(4));
      default: //테스트용.
    ...
    public void InitItem() //아이템타입,공격력,방어력,체력,가격
    {
        ...
        ItemDB.Add(new Item("허름한 조끼", "낡은것 같지만 쓸만할지도?", ItemType.ARMOR, 0, 5, 0, 800));
        ItemDB.Add(new Item("가죽 갑옷", "장인의 손길로 한땀한땀!.", ItemType.ARMOR, 0, 8, 0, 1500));
        ItemDB.Add(new Item("브리간딘", "이제 좀 단단하지 않나요?", ItemType.ARMOR, 0, 12, 0, 2800));
        ItemDB.Add(new Item("사슬 갑옷", "촘촘하게 만들어서 튼튼해요.", ItemType.ARMOR, 0, 16, 0, 3700));
        ItemDB.Add(new Item("미스릴 갑옷", "드워프가 만든건 일품이죠.", ItemType.ARMOR, 0, 22, 0, 5300));
        ItemDB.Add(new Item("드래곤스킨", "전설로 내려오는 갑옷이에요!", ItemType.ARMOR, 0, 30, 0, 7200));
